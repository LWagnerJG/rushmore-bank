...
Production: https://beans-game.vercel.app (`https://roundacats.vercel.app` remains a working alias)
...
- ~1100 curated topics with scope tags (`src/shared/topics.ts`) — prefer basic, fillable prompts.
...
- No response in 45s → W=min(1, max) (still at least 1 when beans exist).
